...
export class GameUIScene extends Phaser.Scene {
    ...
    drawHeadline = (): void => {
        // Headline text 'danger awaits'
        this.headline = this.add.text(this.cameras.main.centerX, this.cameras.main.height * 0.15, 'Danger Awaits', {
            fontSize: '32px',
    ...
    drawAdventurer = (): void => {
        ...
        const width = 0.18
        const height = 0.4
        this.adventurer = this.add.container(this.cameras.main.centerX * xOffset, this.cameras.main.height * yOffset)
        const adventurerPanel = this.add.rectangle(0, 0, this.cameras.main.width * width, this.cameras.main.height * height, COLORS.primary.hex)
        this.adventurer.add(adventurerPanel)

        // Draw Adventurer text (TBD)
        const adventurerText = this.add.text(0, 0, 'adventurer equipment and statistics', {
            align: 'center',
            fontSize: '16px',
        ...
        for (let i = 0; i < this.items.length; i++) {
            ...
            const itemPanel = this.add.rectangle(x, y, itemSize, itemSize, COLORS.primary.hex)
            // Keep track of our array so we can place items in them
            this.itemButtons.push(itemPanel) // Keep track of our array 
            const sword = this.add.sprite(itemPanel.x, itemPanel.y, this.items[i]).setScale(3.5)
    ...
    drawRoom = (): void => {
        ...
        this.roomPanel = this.add.container(this.cameras.main.centerX * xOffset, this.cameras.main.height * yOffset)
        const roomBg = this.add.rectangle(0, 0, this.cameras.main.width * width, this.cameras.main.height * height, COLORS.bg.hex, 0)
            .setStrokeStyle(2, COLORS.primary.hex)
        this.roomPanel.add(roomBg)

        // Room Headline (Room #)
        const roomHeadline = this.add.text(-roomBg.width / 2.5, -roomBg.height / 2.2, 'Room 0', {
            fontSize: '20px',
            color: COLORS.primary.toString()
        })
