...
export class LoadData {
	...
	loadZone = async (zone: string) => {
		...
		for (let i = 0; i < zoneData.entities.length; i++) {
			...
			if (zoneData.entities[i].base) {
				...
				const template = zoneData.entities[i].base
					? this.templates.get(`${zoneData.entities[i].base}.ts`)
					: {}
	...
	getFiles = async (directory: string) => {
		...
		const filesOrDirectories = await fs.readdir(directory, {
			withFileTypes: true,
		})
			...
			for (let j = 0; j < second.length; j++) {
				...
				if (clone != undefined) {
					...
					const index = clone.findIndex(
						(element) => element.type === component.type
					)
